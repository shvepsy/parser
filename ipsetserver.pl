...
while ($client_addr = accept(NEW_SOCKET, SOCKET)) {
   ...
   if ($req_method eq "check" ) {
      #my $cmd = "/home/firewall/firewall.sh ipset | egrep 'Name: BAN|$req_ip' | sed -e 's| timeout.*||g' -e 's|Name: ||g' | grep -P '^\d+' -B 1 | grep -v '^--' | head -n 2";
      my @ipset = `sudo ipset -L | egrep "Name: BAN|$req_ip" | sed -e "s| timeout.*||g" -e "s|Name: ||g" | grep -P "^\d+" -B 1 | grep -v "^--" | head -n 2`;
      #my @ipset = system($cmd);
      print "$?";
      print "@ipset[0] $ipset[1]";
      print NEW_SOCKET "Not ban\n" if (!@ipset);
      print NEW_SOCKET "$ipset[0] banned in $ipset[1] table\n"
    }
    elsif ($req_method eq "del" ) {
      #my $cmd = "/home/firewall/firewall.sh ipset | egrep 'Name: BAN|$req_ip' | sed -e 's| timeout.*||g' -e 's|Name: ||g' | grep -P '^\d+' -B 1 | grep -v '^--' | head -n 2";
      my $cmd = "sudo ipset -L | egrep 'Name: BAN|$req_ip' | sed -e 's| timeout.*||g' -e 's|Name: ||g' | grep -P '^\d+' -B 1 | grep -v '^--' | head -n 2";
      my @ipset = system($cmd);
      #my $cmd = "/home/firewall/firewall.sh ipsetdel $ipset[0] $ipset[1]";
      my $cmd = "sudo ipset del $ipset[0] $ipset[1]";
      my $del = system($cmd);
      print $del;
      if (!$del) { print NEW_SOCKET "Unbanned!\n" } else { print NEW_SOCKET "Not banned\n" };
    }
